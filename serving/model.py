...
# MLflow config (used if available)
...
# Pickle fallback path (baked into Docker image by CI)
...
def _load_from_mlflow() -> bool:
    ...
    try:
        ...
        log.info("Loaded model from MLflow '%s' @%s v%s", REGISTERED_MODEL_NAME, MODEL_ALIAS, _model_version)
        return True
...
def _load_from_pickle() -> bool:
    """Load model.pkl baked into the Docker image by CI."""
    global _model, _model_version
    try:
...
def load_model() -> bool:
    """Try MLflow first, fall back to pickle."""
